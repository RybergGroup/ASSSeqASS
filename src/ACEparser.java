...
public class ACEparser {
    ...
    ACEparser (InputStream in) throws Exception {
	parsACE(in);
    }
//////////////////
/// The parser ///
//////////////////
    public void parsACE ( InputStream in ) throws Exception {
	String word = "";
	char mode = '0';
	...
	while ((c=in.read()) !=-1) {
		...
		if (!word.isEmpty()) {
		    /// Identify ACE entry ///
		    if (newline && word.equals("AS")) { mode = 'B'; System.err.println("At start");}
	    	    else if (newline && contigs !=null && word.equals("CO")) { mode = 'C'; contigs[contigNo] = new Contig(); readNo = 0;}
    		    else if (newline && contigs !=null && contigs[contigNo] != null && word.equals("BQ")) { mode = 'Q'; baseNo= 0; System.err.println("Found q scores");}
		    else if (newline && contigs !=null && contigs[contigNo] != null &&
					reads != null  && word.equals("AF")) { mode = 'A'; System.err.println("Found read " + readNo);}
			...
			if (contigs[contigNo].reads[readNo] == null) {
			    ...
			    System.err.println("Reading read: " + contigs[contigNo].reads[readNo].name);
			}
			...
			if (readNo == this.reads.length) {
			    ...
			    if (readNo == this.reads.length) {
				System.err.println("Could not find read " + word + ", and the given number of reads have been read.");
				return;
			    }
			}
